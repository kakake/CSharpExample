...
namespace miniCoreFrame.DbProvider
{
    public class SqlPage
    {
        ...
        private static string Db2FormatSql(string strsql, PageInfo pageInfo, AbstractDbHelper oleDb)
        {
        ...
        private static string MsAccessFormatSql(string strsql, PageInfo pageInfo, AbstractDbHelper oleDb)
        {
            return null;
        }
        private static string MySQLFormatSql(string strsql, PageInfo pageInfo, AbstractDbHelper oleDb)
        {
            if (pageInfo.KeyName == null || pageInfo.KeyName == "")
                throw new Exception("分页KeyName属性不能为空，如：pageInfo.KeyName==\"Id\" 或 pageInfo.KeyName==\"Id|Desc\"");
